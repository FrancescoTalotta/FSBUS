...
BOOL FsbusOpen (LPSTR dev)
{
    ...
    dcb.StopBits =  2;  			//0;         // 0,1,2 = 1, 1.5, 2
...
//-------------------------------------------------------------------
// wenn cid > 30, wird automatisch ein Byte1 eingef�gt
...
//-------------------------------------------------------------------
BOOL FsbusWriteFmt4(int cid, int rid, int val)
{
    unsigned char buf[5];
	int i = 1;
	int cid0 = cid;
...
// dekodierte Objekte werden mit dem Datum gef�llt und
...
void OnChar(unsigned char b)
{
    ...
    case 3:                 //Added for the ANALOG IN 16 bits
        val |= (b << 8);
        if (!pRdObject)
            break;
        if (!(pRdObject->o.flags & FLG_DISABLED))
            OnAnalogueIn(pRdObject, val);
        break;
    }
...
BOOL MkFsbusObject(FSBUSTYPE ftp, int oid,char* nm,void(* cb)(int oid, int val, double dbl), int c, int r, int flg)
{
    ...
    case BTP_V_OUT:
	    ...
	    if (r < 80 || r >= 90)
		{
			Error ("Object %s (%d) RID:%d out of range(80-89)",po->o.name, oid, rid);
			return FALSE;
		}
        break;
    case BTP_MA_OUT:
	    ...
	    if (r < 236 || r >= 255)
		{
			Error ("Object %s (%d) RID:%d out of range(236-254)",po->o.name, oid, rid);
			return FALSE;
		}
        break;
    }
...
void WriteAnalogueD(FSBUSOBJECT* po, int i32)
{
	if (po->o.flags & FLG_PWROFF)
		i32 = po->uoff.i32;
    FsbusWriteFmt4 (po->cid, po->rid, i32);
}
