...
BOOL CheckIn ()
{
	...
	printf ("%s\r\n", txt);
    printf ("Dirk Anderseck and Francesco Talotta\r\n\r\n");
...
//-------------------------------------------------------------------
bool NewObjectIdOk(int oid, char* name)
{
    if (oid <= 0 || oid >= MAXCONTAINEROBJECTS)
    {
        Error ("Object-ID %d (Group %d, Object %d), Name: %s out of range", oid, oid>>5, oid&0x1f, name);
        return false;
    }

    if (Objects[oid])
    {
        Error ("Object-ID %d (Group %d, Object %d), Name: %s still in use", oid, oid>>5, oid&0x1f, name);
        return false;
    }
    return true;
}
...
void FsbusObjectReport()
{
	...
	printf ("   rotary ..................... %d\r\n", irot);
	printf ("   analog in (8 bits) ......... %d\r\n", iain);
	printf ("   analog in (16 bits) ........ %d\r\n", iaind);
	printf ("   led, digital out ........... %d\r\n", idout);
	printf ("   7 segment display .......... %d\r\n", idsp);
	printf ("   analog out (8 bits) ........ %d\r\n", iaout);
	printf ("   analog out (16 bits) ....... %d\r\n", iaoutd);
	printf ("   stepper .................... %d\r\n", ivout);
...
BOOL MkFsObject(
				...
				int flags)
{
    if (!NewObjectIdOk(oid, nm))
		return FALSE;

    if (datasz < 1 || datasz > sizeof(UVAR))
    {
        Error ("FSUIPC Object: %d(Group %d, Object %d) datasize out of range", oid, oid>>5, oid&0x1f);
        return FALSE;
    }
...
bool FsuipcOpen()
{
	...
	if (!FSUIPC_m_hWnd)
    {
		...
		//Read FSUIPC_m_hWnd from file
		//std::ifstream myfile("C:\\Users\\Ciccio\\Documents\\GitHub\\a320\\windows_hwdn\\build\\Release\\hwnd.txt"); // open the file for reading
		//if (myfile.is_open()) {
	    // int value;
   	    // while (myfile >> std::hex >> value) { // read the hexadecimal values from the file
    	//    std::cout << "Read value: " << value << std::endl;
		// }
		// FSUIPC_m_hWnd=HWND(value);	
